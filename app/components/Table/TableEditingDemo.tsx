    ...
    setData((prevData) => {
      ...
      if (rowData) {
        if (
          ...
          const typedRow = rowData as Record<string, string | number>;
          typedRow[columnId] = value as string;
        } else if (columnId === "age" || columnId === "id") {
          const numValue =
            typeof value === "string" ? parseInt(value, 10) : (value as number);
          const typedRow = rowData as Record<string, number | string>;
          typedRow[columnId] = numValue;
  ...
  return (
    <div>
      ...
      <div className={styles.editingInfo}>
        Click on any cell to edit its value. Press Enter to save or Escape to
        cancel.
      </div>
      ...
      <Table
        ...
        exportFilename="people-data-editable"
        pageSize={10}
      />
    </div>
  );
